...
const defaultPort = 23004;
const defaultHost = '127.0.0.1';
const defaultDbFile = path.join(appRoot, 'data', 'album.sqlite');
const manifestPath = path.join(appRoot, 'smallphone.app.json');
const publicDir = path.join(appRoot, 'public');

function resolveDbFile(value = process.env.ALBUM_DB_FILE) {
  if (!value) return defaultDbFile;
  return path.resolve(process.cwd(), String(value));
}

function resolveDataFile(value = process.env.ALBUM_DB_FILE) {
